...
class ReportController(ReportController):
    ...
    def report_routes(self, reportname, docids=None, converter=None, **data):
        ...
        for report in report_xml.browse(report_ids):
            ...
            split_fname = generated_filename.split('.')
            if split_fname[0].endswith('False'):
                if objects.partner_id and objects.partner_id.is_company:
                    split_fname[0] = split_fname[0].replace('_False', "_" + objects.partner_id.name)
                else:
                    split_fname[0] = split_fname[0].replace('_False', "")
            if split_fname[0].startswith('False'):
                split_fname[0] = split_fname[0].replace('False_', "")
            generated_filename = '.'.join(split_fname)
#             if 'False_' in generated_filename:
#                 generated_filename = generated_filename.replace('False_', "")
